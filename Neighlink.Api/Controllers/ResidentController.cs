...
{
    [ApiController]
    [Route(ConstantHelper.API_PREFIX + "/residents")]
    ...
    public class ResidentController : BaseController
    {
        ...
        public ResidentController(NeighlinkContext context)
        {
            this._context = context;
        }

        private IQueryable<Residents> PrepareQuery() => _context.Residents
         .OrderBy(x => x.Id)
         .AsQueryable();

        //[HttpGet]
        //[ProducesResponseType(typeof(DefaultResponse<CollectionResponse<UserResponse>>), StatusCodes.Status200OK)]
        //public IActionResult GetAll([FromQuery] ResidentGetRequest model)
        //{
        //    try
        //    {
        ...
        //            query = query.Where(x => x.User.Name.Contains(model.Name));

        //        var dtos = ServiceHelper.PaginarColeccion(HttpContext.Request, model.Page, model.Limit, query,
        //          pagedEntities => ResidentResponse.Builder.From(pagedEntities).BuildAll());
